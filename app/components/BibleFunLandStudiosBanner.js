...
export default function BibleFunLandStudiosBanner({ className = '' }) {
  return (
    <a
      ...
      className={`group flex items-center justify-center gap-2 px-4 py-2 text-center transition-all duration-300 border-b border-white/10 bg-black/45 backdrop-blur-xl hover:bg-black/65 hover:border-amber-400/25 ${className}`}
    ...
    >
      ...
      <span className="text-[10px] font-bold text-teal-400/80 group-hover:text-teal-300 transition-colors shrink-0">
        Explore →
      </span>
    </a>
  );
